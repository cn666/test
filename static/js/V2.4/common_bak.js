...
Public = {
    ...
    prompt: function (title, msg, ok) {
        layer.prompt({title: msg}, function (text, index) {
            layer.close(index)
            ok(text)
        })
    },
    ...
    $.fn.mydoubledate = $.fn.mydoublePicker = function (options, arg) {
        ...
        function create(target, option) {
            ...
            if (!option) {
                ...
                $.data(target, 'mydoublePicker', {options: option});

                var panelPrev;
                var combox1 = $(target).combo({
                    hasDownArrow: false,
                    editable: false,
                    ...
                    separator:',',
                    multiple: true,
                    panelWidth: 0,//option.panelWidth,
                    panelHeight: 0,//option.panelHeight,
                    onShowPanel: function () {
                        var option = $(this).mydoublePicker('options');
                        var panel = $(this).combo('panel');
                        /*panelPrev = option.panel.prev();
                        var panel = $(this).combo('panel').append('<div></div>');
                        option.panel.insertAfter(panel.children());
                        option.panel.prev().remove();
                        option.panel.css('left', '0px');
*/
                        /*var values=$(this).mydoublePicker('getValue');
                        if(values.length==2){
                        ...
                        if (option.full) {
                            panel.height(option.panelHeight + 30)
                        } else {
                            panel.height(option.panelHeight)
                        }

                        if (option.target.setTimePos) {
                            clearTimeout(option.target.setTimePos);
                        }
                        option.target.setTimePos = setInterval(function () {
                            setPos(option);
                        }, 200);
                        // WdatePicker(option.picker1);
                        // WdatePicker(option.picker2);
                        // console.log(option.picker1)
                        // console.log(option.picker2)
                    },
                    onHidePanel: function () {

                        var option = $(this).mydoublePicker('options');
                        //option.panel.css('visibility','hidden');
                        /*option.panel.insertAfter(panelPrev);
                       option.panel.css('left', '-10003em');*/
                ...
                textbox.width(function () {
                    return $(this).parent().width() - 32;
                });
                textbox.parent().addClass('date');

                option['combopanel'] = combopanel;
                option['textbox'] = textbox;
                option['target'] = target;
                var source = '' +
                    '<div class="doubledate-panel" style="width:390px;visibility:hidden;position: absolute;top:0px;left:0px;z-index:999999;background:#FFFFFF;border: 1px solid #ccc;">'
                    //控制区域
                    ...
                    + '                <input type="button" value="清 空" class="bt_yellow  doubletime-clear" style="margin-left:5px\\0;margin-left:30px;width:60px;min-width:60px;padding:0px 5px;">'
                    +'                 <input type="button" value="确 定" class="bt_blue doubletime-ok" style="margin-left:5px\\0;margin-left:5px;width:60px;min-width:60px;padding:0px 5px;">'
                    + '          </form>' +
            ...
            var values=$(target).mydoublePicker('getValue');
            $(target).mydoublePicker('setValue',values);
    ...
    $.fn.mydoublePicker.methods = {
        ...
        setDate: function (jq, value) {
            $.data(jq[0], 'date', value);
        },
        getValue: function (jq) {
            return $(jq).combo('getValues');
        },
        clear: function (jq) {
            return jq.each(function () {
